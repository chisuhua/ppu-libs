...
// TODO mlvm  for clc math

#define FUNC2_CLC_D(root) \
  ATTR double __nv_##root(double x, double y) { return __ocml_##root##_f64(x, y); }
#define FUNC2_CLC_F(root) \
  ATTR float __nv_##root##f(float x, float y) { return _Z9__clc_##root##ff(x, y); }
#define FUNC2_CLC(root) FUNC2_CLC_D(root) FUNC2_CLC_F(root)

#define FUNC1_CLC_D(root) \
  ATTR double __nv_##root(double x) { return __ocml_##root##_f64(x); }
#define FUNC1_CLC_F(root) \
  ATTR float __nv_##root##f(float x) { return _Z10__clc_##root##f(x); }
#define FUNC1_CLC(root) FUNC1_CLC_D(root) FUNC1_CLC_F(root)
