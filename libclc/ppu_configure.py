...
available_targets = {
  ...
  'ppu--'   : { 'devices' : [{'gpu' : '', 'aliases' : []} ]},
  'ppu64--' : { 'devices' : [{'gpu' : '', 'aliases' : []} ]},
}
...
#default_targets = ['ppu--', 'ppu64--', 'r600--', 'amdgcn--', 'amdgcn--amdhsa']
...
for target in targets:
  ...
  for device in available_targets[target]['devices']:
    ...
    clang_target = target;
    if target == 'ppu--':
        clang_target = 'amdgcn--amdhsa'
    clang_bc_flags = "-target %s -I`dirname $in` %s " \
                     "-fno-builtin -Xclang -pic-is-pie -Xclang -mrelocation-model -Xclang pic " \
                     "-D__CLC_INTERNAL " \
                     "-emit-llvm" % (clang_target, clang_cl_includes)
    if device['gpu'] != '':
